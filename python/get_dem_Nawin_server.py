...
if __name__ == "__main__":
    ...
    data_disk = '/disk/solar/nn2/data/2018/10/31/0/AIA'
    output_dir = '/disk/solar/nn2/results/DEM/'
    ...
    ## Download the data
    files = glob.glob(data_disk+str(passband[0])+'/*.fits')
    if len(files) == 0:
    ...
    meanEM_lead_outer = [None]*len(flist[index])
    meanEM_lead_inner = [None]*len(flist[index])
    meanEM_fol_outer = [None]*len(flist[index])
    meanEM_fol_inner = [None]*len(flist[index])
    meanT_lead_outer = [None]*len(flist[index])
    meanT_lead_inner = [None]*len(flist[index])
    meanT_fol_outer = [None]*len(flist[index])
    meanT_fol_inner = [None]*len(flist[index])
    meanDens_lead_outer = [None]*len(flist[index])
    meanDens_lead_inner = [None]*len(flist[index])
    meanDens_fol_outer = [None]*len(flist[index])
    meanDens_fol_inner = [None]*len(flist[index])
    ...
    for img in range(0, len(flist[index])):
        ...
        #Load array of CHB and x-ypixel
        arraysaveloc = '/disk/solar/nn2/results/arrayfile/'
        CHB_all = np.load(arraysaveloc+'CHB_all.npz')
        CHB = CHB_all['CHB']
        CHB_in = CHB_all['CHB_in']
        CHB_out = CHB_all['CHB_out']
        landf_pix = np.load(arraysaveloc+'leadandfollow_pix.npz')
        ...
        submap = get_submap(time_array,index,img,f_0193)
        if img == 0:
                submapref = submap
        ...
        np.savez(output_dir+'EM_array_'+dt.datetime.strftime(time_array[index][img], "%Y%m%d_%H%M%S")+'.npz',EM_total = EM_total)
